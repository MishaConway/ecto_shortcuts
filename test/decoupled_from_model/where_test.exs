defmodule Decoupled.WhereTest do
  ...
  test "same query as above but preloading user_status returns users with user_status association preloaded using wildcard" do
    users = MyApp.Users.where [user_status_id: 2], limit: 5, preload: "*"
    Enum.each users, fn(user) ->
      assert 2 == user.user_status.id
    end
  end
